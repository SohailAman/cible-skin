...
for (let i = 0; i < morePara.length; i++) {
  ...
  readMoreBtn[i].addEventListener('click', () => {
    if (viewed === false) {
      ...
      readMoreBtn[i].innerHTML = "Afficher moins";
      viewed = true;
    }
    else {
...
let mySwiperStepsWhole = document.querySelector(".mySwiperSteps");
let mySwiperStepsSlide = document.querySelector("#fifthSlide");

console.log(mySwiperStepsSlide)

function overlayHid() {
  if (mySwiperStepsSlide.classList.contains('swiper-slide-active')) {
    console.log('bib')
    mySwiperStepsWhole.classList.add('gone')
  }
  else {
